...
def generate_summary():
    try:
        analysis_path = os.path.join(UPLOAD_DIR, "analysis_report.json")
        model_path = os.path.join(UPLOAD_DIR, "model_report.json")
        ...
        if not os.path.exists(analysis_path):
            return {"error": "analysis_report.json missing"}

        if not os.path.exists(model_path):
            return {"error": "model_report.json missing"}
...
Write a short, beginner-friendly summary.
"""
